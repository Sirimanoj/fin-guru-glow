...
create policy "Avatar library is readable by everyone"
...
create policy "Users can view their own fin score"
  on public.fin_score for select
  using (auth.uid() = user_id);
create policy "Users can insert their own fin score"
  on public.fin_score for insert
  with check (auth.uid() = user_id);
create policy "Users can update their own fin score"
  on public.fin_score for update
  using (auth.uid() = user_id);
create policy "Users can delete their own fin score"
